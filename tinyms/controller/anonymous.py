    ...
    def post(self, *args, **kwargs):
        ...
        if Utils.is_email(Utils.trim(login_id)):
            rows = cnn.query(Account.id, Archives.name).outerjoin(Archives, Account.archives_id == Archives.id) \
                .filter(Archives.email == login_id).filter(Account.login_pwd == Utils.md5(login_pwd)).filter(
                Account.enabled == 1).limit(1).all()
            if len(rows) > 0:
                id = rows[0][0]
                name = rows[0][1]
            ...
            rows = cnn.query(Account.id, Archives.name).outerjoin(Archives, Account.archives_id == Archives.id) \
                .filter(Account.login_name == login_id).filter(Account.login_pwd == Utils.md5(login_pwd)).filter(
                Account.enabled == 1).limit(1).all()
            if len(rows) > 0:
                id = rows[0][0]
                name = rows[0][1]
